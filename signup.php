			...
			if(isset($_POST['submit'])){
				...
				if($error_flag == false){
					...
					if(mysqli_num_rows($user_exists_result) == 0){
						$user_insert_query = 'INSERT INTO students VALUE ("' .$_POST['firstName']. '","' . $_POST['lastName'] . ',"' . $_POST['phone']. '");';
				...
				if(isset($_POST['dance_class'])){
					if(is_array($_POST['dance_class'])){
						foreach($_POST['dance_class'] as $c){
							$signup_query = 'INSERT INTO class_signups VALUE(' .$c. ',"' .$_POST['firstName']. '","' .$_POST['lastName']. '","' .$_POST['phone']. '");';
